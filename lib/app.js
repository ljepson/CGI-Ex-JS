...
(function(root, factory) {
    ...
    if ('function' === typeof define && define.amd) {
        define(['underscore', './template', './cgix', './validate', './conf'], factory);
    }
        ...
        root.App = factory(root._, root.Template, root.CGIEx, root.Validate, root.Conf);
    }
}(this, function(_, Template, CGIEx, Validate, Conf) {
    'use strict';

    var App = function(req, res) { // jshint ignore:line
        // Store request and response for later use
        this.__req = req;
        this.__res = res;

        /* Error-handling */
        this.AppError = function(error_obj) {
            function AppError() {
        ...
        this.find_hook = function find_hook(hook, step) {
            ...
            if (step) {
                ...
                // No step provided, just look for the hook by name
                var code = this.can(hook);
        ...
        this.path_info = function path_info(pathname) {
            ...
            // Default path - should be set by the application
            this._path_info = '/';
        ...
        this.conf_obj = function conf_obj() {
            if (this._conf_obj) {
                return this._conf_obj;
            }

            if ('undefined' !== typeof Conf) {
                var args = this.conf_args();
                this._conf_obj = new Conf(args);
        ...
        this.val_obj = function val_obj() {
            ...
            if ('undefined' !== typeof Validate) {
                var args = this.val_args();
                this._val_obj = new Validate(args);
        ...
        this.cgix = function cgix() {
            ...
            if ('undefined' !== typeof CGIEx) {
                this._cgix = new CGIEx(this.__req);
        ...
        this.cookies = function cookies(c) {
            ...
            if (!this._cookies) {
                var cgix = this.cgix();
                this._cookies = cgix ? cgix.get_cookies() : {};
            }
        ...
        this.conf = function conf(data) {
            ...
            var conf_file = this.conf_file();
            var conf_obj = this.conf_obj();
            var config;

            if (conf_file && !_.isObject(conf_file) && conf_obj) {
                config = conf_obj.read(conf_file, {no_warn_on_fail: 1}) || false;

                if (!config && this.conf_die_on_fail()) {
                    return this.croak('Could not read config file: ' + conf_file);
                }
            }

            var conf_validation = this.conf_validation();

            if (config && conf_validation && _.keys(conf_validation).length) {
                var val_obj = this.val_obj();

                if (val_obj) {
                    var error_obj = val_obj.validate(config, conf_validation);

                    if (error_obj) {
                        return this.croak(error_obj.as_string());
                    }
                }
            }
        ...
        this.add_to_swap = function add_to_swap(data) {
            var swap = this.hash_swap();
            return _.extend(swap, data);
        };
        ...
        this.add_errors = function add_errors(errors) {
        ...
        this.add_to_hash = function add_to_hash(old_obj, new_obj) {
            // Use underscore's extend instead of jQuery's $.extend
            return _.extend({}, old_obj, new_obj);
        };
        ...
        this.jump = function jump(step) {
            ...
            // Replace current path with the jump destination
            this._path = _.isArray(step) ? step : [step];
            this._path_i = 0;

            // Trigger navigation loop again
            this.nav_loop();
        };

        this.goto_step = function goto_step(step) {
            if (!step) {
                return;
            }

            // Clear everything after current position and insert new step
            var path = this.path();
            var i = this._path_i || 0;
            ...
            // Don't increment path_i - we'll continue from current position
        };

        this.js_uri_path = function js_uri_path() {
        };

        this.morph = function morph(step) {
            // Morphing allows changing the object's behavior for specific steps
            // This is a simplified implementation - full morph would use prototypal inheritance
            if (!this._morph_lineage) {
                this._morph_lineage = [];
            }

            var allow = this.run_hook('allow_morph', step);

            if (!allow) {
                return;
            }

            var package_name = this.run_hook('morph_package', step);

            if (package_name && package_name !== this.constructor.name) {
                this._morph_lineage.push({
                    step: step,
                    package: package_name
                });

                this.fixup_after_morph(step);
            }
        };

        this.replace_path = function replace_path(steps) {
            var steps_array = _.isArray(steps) ? steps : [steps];
            var path = this.path();
            var i = this._path_i || 0;

            // Replace current step with new steps
            [].splice.apply(path, [i, 1].concat(steps_array));

            // Decrement path_i so we re-run the first new step
            this._path_i = Math.max(0, i - 1);
        };

        this.set_path = function set_path(path) {
            this._path = _.isArray(path) ? path : [path];
            this._path_i = 0;
        };
        ...
        this.unmorph = function unmorph(step) {
            if (!this._morph_lineage || this._morph_lineage.length === 0) {
                return;
            }

            var start_index = this._morph_lineage_start_index || 0;

            while (this._morph_lineage.length > start_index) {
                var morph = this._morph_lineage.pop();

                this.fixup_before_unmorph(step);

                // In a full implementation, would restore previous prototype
                // For now, just track the lineage
            }
        };
        ...
        this.print_out = function print_out(step, out) {
            // This is handled by the application layer (see app.js)
            // Subclasses should override this to handle output appropriately
        };
        ...
        this.get_template = function get_template(file) {
            ...
            if (!this._templates[file]) {
                // Template reading is handled by template_obj
        ...
        this.validate = function validate(step, form) {
            ...
            if (!val_obj) {
                return 1; // No validator available, skip validation
            }

            var validated_fields = [];
            var err_obj = val_obj.validate(form, hash, validated_fields);
