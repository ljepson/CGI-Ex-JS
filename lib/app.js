...
(function(root, factory) {
    ...
    var App = function(req, res) { // jshint ignore:line
        ...
        this.AppError = function(message) {
            if (this._AppError) {
                throw new this._AppError(message);
            }
            ...
            this._AppError = AppError.prototype.constructor = AppError;
        ...
        this.die = function die(error) {
            ...
            // Received an object as the error
            if (_.isObject(error)) {
                var error_keys = ['errno', 'message', 'msg'];
                ...
                if (found_key && 'errno' === found_key) {
                    ...
                    error_message = (34 === error.errno) ?
                        ('Unable to open file ' + error.path) : '';
                }
                else if (found_key) {
                    error_message = error[found_key];
                }

                // Throw the resolved error message or the default one
                if (error_message) {
                    console.log('error_message', error_message);
                    throw new this.AppError(error_message || default_message);
                }
            }

            // Handle all other cases
            throw new this.AppError(
                _.isString(error) ? error : default_message
            );
        };
        ...
        this.navigate = function navigate(req, res) {
            ...
            // Don't think this catch block is being handled correctly...
                ...
                if (!this.can('handle_error')) {
                    this.die(err);
                }

                handled_error = this.eval(this.handle_error, err);
            }

            // Investigate the usefulness of this
            if (handled_error.error) {
                this.die(handled_error.message);
            }
            else if (handled_error.result && 'AppError' === handled_error.result.name) {
                var handled_message = handled_error.result.message;

                if (handled_message) {
                    this.die(handled_message);
                }
            }
        ...
        this.run_hook = function run_hook(hook, step) {
            ...
            if (has_exception && response.message) {
                this.die(response.message);
            }
        ...
        this.print = function print(step, swap, fill) {
            ...
            var response = this.run_hook('swap_template', step, file, swap);
            var out = response.result;

            if (response.error) {
                return this.die(response.result);
            }
